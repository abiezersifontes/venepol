...
function Header()
{
    ...
    $this->Cell(80);
    // Título
    $this->Cell(50,10,'Acta de Novedad',0,0,'C');
    // Salto de línea
    $this->Ln(20);
}
...
$con = conectar();
$q1 = mysqli_query($con, "SELECT * FROM novedad WHERE id_novedad = '".$_GET['id_novedad']."'");

while($dato = mysqli_fetch_array($q1)){
	$id_novedad = $dato['id_novedad'];
	$fecha = $dato['fecha'];
	$hora = $dato['hora'];
...
Novedad Nº".$id_novedad."
                                                                                                                  Ciudad Bolívar, ".$fecha."
 
        En esta misma fecha, siendo las ".$hora." Se Registro una novedad con la siguiente descripcion: ´´".$descripcion."´´
